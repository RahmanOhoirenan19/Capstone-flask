...
playstore.Price = playstore.Price.apply.apply(lambda x: x.replace('$',''))
...
def index():
    ...
    stats = {
        ...
        'Reviews':'mean','Rating':'max'}).sort_values("Reviews",ascending=False).reset_index().to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
